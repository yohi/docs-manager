...
const CommandInterceptorPlugin: Plugin \= async (ctx) \=\> {  
  return {  
    ...
    "chat.message": async (params, { message }) \=\> {  
      ...
      const COMMAND\_TRIGGER \= "/deploy"; // 捕捉したいコマンド

      // 2\. コマンド構文の検出  
      // startsWithによる単純な判定、あるいは正規表現を使用  
      if (content.trim().startsWith(COMMAND\_TRIGGER)) {  
          
        // 引数の抽出  
        const args \= content.slice(COMMAND\_TRIGGER.length).trim();  
...
1. **キャッシュ確認**: \~/.cache/opencode/node\_modules/ に該当パッケージがあるか確認。  
